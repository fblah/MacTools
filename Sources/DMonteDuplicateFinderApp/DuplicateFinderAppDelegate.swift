...
private final class DuplicateFinderStatusView: NSControl {
    ...
    init() {
        image = NSImage(systemSymbolName: "doc.on.doc", accessibilityDescription: "Duplicate Finder") ?? NSImage()
...
@MainActor
final class DuplicateFinderAppDelegate: NSObject, NSApplicationDelegate, NSWindowDelegate {
    ...
    private func configureWindow() {
        let windowSize = DuplicateFinderSizing.preferredSize()
    ...
    private func configureStatusItem() {
        let item = NSStatusBar.system.statusItem(withLength: DuplicateFinderStatusView.statusWidth)
        statusItem = item

        let statusView = DuplicateFinderStatusView()
    ...
    private func configureWindowShowNotifications() {
        DistributedNotificationCenter.default().addObserver(
            ...
            name: Notification.Name("com.havokentity.mactools.duplicatefinder.showWindow"),
            object: nil
        )
    }
