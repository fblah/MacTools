...
private final class ImageConverterStatusView: NSControl {
    ...
    init() {
        image = NSImage(
            ...
            accessibilityDescription: "Image Converter"
        ) ?? NSImage()
...
@MainActor
final class ImageConverterAppDelegate: NSObject, NSApplicationDelegate, NSWindowDelegate {
    /// Distributed notification the Toolbox (and a second `--open` launch) posts to
    /// bring the window forward. Matches the bundle id namespace by convention.
    static let showWindowNotification = Notification.Name("com.havokentity.mactools.imageconverter.showWindow")
    ...
    private func configureWindow() {
        let windowSize = ImageConverterSizing.preferredSize()
