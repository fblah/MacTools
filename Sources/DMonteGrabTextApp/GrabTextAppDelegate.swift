...
private final class GrabTextStatusView: NSControl {
    ...
    init() {
        image = NSImage(systemSymbolName: "text.viewfinder", accessibilityDescription: "Grab Text") ?? NSImage()
...
@MainActor
final class GrabTextAppDelegate: NSObject, NSApplicationDelegate, NSWindowDelegate {
    ...
    private func configureWindow() {
        let windowSize = GrabTextSizing.preferredSize()
        ...
        window.level = .normal
        window.title = "DMonte Grab Text"
        self.window = window
    }

    private func configureStatusItem() {
        let item = NSStatusBar.system.statusItem(withLength: GrabTextStatusView.statusWidth)
        statusItem = item

        let statusView = GrabTextStatusView()
    ...
    private func configureWindowShowNotifications() {
        DistributedNotificationCenter.default().addObserver(
            ...
            name: grabTextShowWindowNotification,
            object: nil
        )
    }
