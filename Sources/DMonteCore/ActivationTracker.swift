...
public final class ActivationTracker: NSObject {
    ...
    public nonisolated static let spuriousActivationWindow: TimeInterval = 0.5
    ...
    public init(logger: Logger, excluding isExcluded: @escaping @MainActor (NSRunningApplication) -> Bool) {
        self.log = logger
    ...
    @objc private func applicationDidActivate(_ note: Notification) {
        ...
        record(Entry(pid: app.processIdentifier, name: app.localizedName, at: Date()))
    }
    ...
    public func resolveTarget(now: Date = Date()) -> NSRunningApplication? {
        ...
        case .skippedSuspicious(let target, let skipped):
            ...
            return Self.liveApp(target.pid)
        }
    }
